...
function run_slotted_aloha_dc_snr_sweep(sf::Int, bw::Float64, num_devices::Int,
    ...
    open(save_path, "w") do io
        println(io, "SNR_dB,PER,Collision_Rate,SNR_Fail_Rate,DC_Blocked_Rate")
        for i in 1:length(snrs)
            println(io, "$(snrs[i]),$(per_vals[i]),$(collision_rates[i]),$(snr_fail_rates[i]),$(dc_blocked_rates[i])")
        end
    end
    
    @info "DC制約付きSlottedALOHA SNRスイープ結果をCSVに保存しました: $save_path"
    
    return snrs, per_vals, collision_rates, snr_fail_rates, dc_blocked_rates
end
...
snrs_dc, per_vals_dc, collision_rates, snr_fail_rates, dc_blocked_rates = run_slotted_aloha_dc_snr_sweep(sf, bw, num_devices,
