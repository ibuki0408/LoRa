...
# ===== シャドウイング計算（決定論的） =====
function calculate_shadowing(shadowing_params::ShadowingParameters, distance_m::Float64, fixed_value::Bool=false, seed::Int=1234)
...
# ===== OFDM信号生成（QPSK変調・決定論的） =====
function generate_ofdm_signal_with_qpsk(bandwidth_hz::Float64, duration_s::Float64, fs_hz::Float64, tx_power_dbm::Float64, seed::Int=1234)
    # ランダムシードを固定
    Random.seed!(seed)
...
# ===== 周期的同期信号生成 =====
function generate_periodic_sync_signals(params::SignalParameters, interval_ms::Float64, total_duration_ms::Float64, base_seed::Int=1234)
    ...
    for signal_time in signal_times
        # 信号生成（各信号で完全にランダムなシードを使用）
        random_signal_seed = Int(round(time() * 1000000)) % 1000000 + signal_count * 1000
        signal_samples = generate_ofdm_signal_with_qpsk(params.bandwidth_hz, params.duration_s, params.sampling_rate_hz, params.tx_power_dbm, random_signal_seed)
...
# ===== パスロス・シャドウイングのみ（ノイズなし） =====
function add_path_loss_only(tx_signal::Vector{ComplexF64}, path_loss_params::PathLossParameters, 
                           shadowing_params::ShadowingParameters, sampling_rate_hz::Float64, 
    ...
    println("受信側パスロス・シャドウイングパラメータ（ノイズなし）:")
    ...
    println("• 受信信号電力: $(round(signal_power_dbm, digits=2)) dBm")
    println("• ノイズ: なし")
    println()
    
    return rx_signal_with_path_loss, path_loss_db
end

# ===== 端末配置（決定論的） =====
function deploy_terminals(deployment_params::TerminalDeploymentParameters, shadowing_params::ShadowingParameters, tx_power_dbm::Float64, seed::Int=1234)
    # ランダムシードを固定
    Random.seed!(seed)
    ...
    if deployment_params.deployment_mode == "poisson"
        ...
        for i in 1:num_points
            ...
            if distance >= deployment_params.min_distance_m && distance <= deployment_params.max_distance_m
                ...
                # シャドウイング計算（決定論的）
                shadowing_db = calculate_shadowing(shadowing_params, distance, false, seed)
...
function save_received_power_to_csv(time_axis::Vector{Float64}, rx_power::Vector{Float64}, 
    ...
    CSV.write("$(output_dir)/received_power_data_no_noise_$(timestamp).csv", power_data)
    
    # パラメータデータ
    params_data = DataFrame(
        parameter = [
            "signal_duration_us", "center_frequency_ghz", "signal_bandwidth_mhz", "terminal_bandwidth_mhz",
            "tx_sampling_rate_mhz", "rx_sampling_rate_mhz", "tx_power_dbm",
            ...
            "total_duration_ms", "power_threshold_dbm", "detection_rate_percent", "noise_enabled"
        ],
        value = [
            params.duration_s * 1e6, params.center_frequency_hz / 1e9, params.bandwidth_hz / 1e6, 0.125,
            61.44, 0.0025, params.tx_power_dbm,
            shadowing_params.enabled, shadowing_params.std_db, "fixed", length(terminals),
            100.0, 10.0, 500.0, 3.0, 1.0, 0.0, 20.0, 110.0, -100.0, detection_rate, false
        ]
    )
    CSV.write("$(output_dir)/QPSK_simulation_parameters_no_noise_$(timestamp).csv", params_data)
end
...
function create_simulation_parameters()
    return SimulationParameters(
        ...
        4.7,                # 中心周波数（GHz）
        3.6,                 # 同期信号帯域幅（MHz）- 固定
        3.6,               # 端末受信帯域幅（MHz）- LoRa標準帯域幅
        7.68,                # 送信側サンプリングレート（MHz）- ナイキストの定理満足
        7.68,                # 受信側サンプリングレート（MHz）- 端末受信帯域幅の2倍
        43.0,               # 送信電力（dBm）
...
# ===== リサンプリング対応版メイン実行関数（ノイズなし） =====
function main_resampling_no_noise()
    # 実行毎に異なるシードを使用（現在時刻ベース）
    current_seed = Int(round(time() * 1000)) % 1000000
    Random.seed!(current_seed)
    println("使用シード: $(current_seed)")
    
    println("="^60)
    title_str = "同期信号受信シミュレーション（リサンプリング対応版・ノイズなし）"
    println(title_str)
    println("="^60)
    ...
    # 端末配置
    println("端末配置を生成中...")
    # 実行毎に異なるが、同一実行内では固定のシャドウイング
    terminal_seed = 1234 + (current_seed % 1000)  # 実行毎に異なるが、同一実行内では固定
    terminals = deploy_terminals(deployment_params, shadowing_params, sim_params.tx_power_dbm, terminal_seed)
    ...
    signal_seed = current_seed + 2000  # 信号生成用の異なるシード
    time_axis_tx, tx_signal_high_rate, signal_count = generate_periodic_sync_signals(
        params_tx, sim_params.signal_interval_ms, sim_params.total_duration_ms, signal_seed
    )
    ...
    rate_ratio = sim_params.rx_sampling_rate_mhz / sim_params.tx_sampling_rate_mhz # レート比を計算 (例: 0.0025 / 61.44)
    ...
    # ===== 5. ダウンサンプリング後の信号に対してパスロス・シャドウイングのみ適用（ノイズなし） =====
    # 時間軸も受信側のレートに合わせて再計算
    ...
    println("受信側：パスロス、シャドウイングのみ適用中（ノイズなし）...")
    # ★ ダウンサンプリング後の信号に対してパスロス・シャドウイングのみ適用
    rx_signal, path_loss_db = add_path_loss_only(
        tx_signal_low_rate, representative_path_loss_params, shadowing_params, sim_params.rx_sampling_rate_mhz * 1e6, 
    ...
    # 出力ディレクトリを作成
    output_dir = "results_QPSK_simulation_resampling_no_noise"
    mkpath(output_dir)
    ...
    save_received_power_to_csv(time_axis_rx, rx_power, peak_times, peak_powers, 
                              params_tx, terminals, representative_terminal,
                              signal_count, detection_rate, output_dir, shadowing_params)
    println("CSVファイルを '$(output_dir)' に保存しました。")
    println("• received_power_data_no_noise_$(execution_timestamp).csv - 受信電力データ（mW, dBm）")
    println("• QPSK_simulation_parameters_no_noise_$(execution_timestamp).csv - パラメータ情報")
    println()
