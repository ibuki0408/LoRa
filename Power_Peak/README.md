...
*   **`path_loss.jl`**: 距離と周波数に応じた電波減衰を計算します。
*   **`shadowing.jl`**: 障害物による電波強度のばらつき（シャドウイング）を模擬します。
...
実行すると、コンソールにシミュレーション経過が表示され、`result_integrated/` フォルダに結果ファイルが生成されます。

## シミュレーションの流れ

1.  **5G同期信号の受信** (3.7GHz, 43dBm)
    *   端末はランダムな時刻に起動 (`Startup Time`)
    *   起動後、最初に来たビーコン信号を検出して同期
2.  **LoRaデータ通信** (920MHz, 13dBm)
