...
function create_integrated_params()
    ...
    # --- ACK/再送設定 ---
    enable_ack = false     # true: 再送あり, false: 再送なし
    
    # --- 比較実験用: 非同期モード ---
    force_async_mode = false  # true: 全端末を同期失敗扱い（LBT-ALOHA比較用）
    ...
    return IntegratedParameters(
        ...
        200.0,    # initial_window_duration_ms (ビーコン間隔の2倍)
        ...
        # === ACK/再送 ===
        enable_ack,     # enable_ack (テストのため有効化)
        3,        # max_retries
        ...
        force_async_mode,  # force_async_mode
        10        # max_buffer_size (デフォルト10個)
    )
end
...
function perform_hifi_synchronization(terminals, beacon_times, sig_gen_params, params, output_dir)
    ...
    for t in terminals
        ...
        # --- NEW: フィルタ付きリサンプリング (DSP.resample) ---
        # 必要な信号のみを取り出すため、エイリアシングを防ぎ、帯域外ノイズもカットされます。
        # 注意: 計算コストが増加します。
        # sig_rx_segment = DSP.resample(sig_tx_segment, 1/downsample_ratio)
        ...
        # --- NEW: 自動反映 ---
        # resampleフィルタにより帯域外電力は既にカットされているため、
        # 追加の損失計算は不要（または 0dB）とします。
        # bw_mismatch_loss_db = 0.0
    ...
    # 詳細ログが有効な場合のみ保存
    if params.enable_detailed_logs && params.enable_file_output
        out_path = joinpath(output_dir, "prop_sync_log_$(params_str)_$(timestamp).csv")
    ...
    if nrow(synced_data) > 0
        ...
        if params.enable_file_output
            accuracy_path = joinpath(output_dir, "prop_sync_accuracy_$(params_str)_$(timestamp).csv")
...
function schedule_tx_start!(event_queue, me, packet_id, base_time, params, terminal_sync_infos, synced_terminal_ids)
    # ★ バースト衝突緩和のためのランダムバックオフ ★
    # DC明けやキュー処理の際、全員が一斉に送信するのを防ぐ
    backoff_ms = rand() * params.backoff_base_ms
    effective_base_time = base_time + backoff_ms
...
function run_integrated_simulation_with_params(params::IntegratedParameters)
    ...
    if params.packet_airtime_ms == 0.0
        ...
        params = IntegratedParameters(
            ...
            params.force_async_mode,  # force_async_mode
            ...
            if is_successful
                ...
                for inf in interferers
                    ...
                    if inf_term_id != me_id
                        ...
                        # 隠れ端末判定 (Audibility Check)
                        # 干渉端末からの信号が、自分のLBT閾値未満だったか？
                        # パスロス計算 (端末間)
                ...
                if params.enable_ack && rec.retry_count < params.max_retries
                    # 再送スケジュール
                    backoff = rand() * 4000.0
                    schedule_tx_start!(event_queue, me, pid, max(curr_t + backoff, next_available_time[me_id]), params, terminal_sync_infos, synced_terminal_ids)
                else
                    # 完全に失敗
                    rec.status = "Dropped"
                    # キューから削除
    ...
    for r in finished_tx
        if r.status == "Success"
            push!(success_pid_set, r.original_packet_id)
        end
    end
    ...
    # 衝突距離のビニング (0-100m, 100-200m, ..., 900-1000m, >1000m)
    distance_bins = collect(0:100:2000)  # [0, 100, 200, ..., 2000]
    ...
    retransmission_rate = total_attempts > 0 ? (total_retransmissions / total_attempts) : 0.0
