...
# 1. 必要なファイルを読み込み
@everywhere include("Prop_Async_Optimized.jl")
...
function evaluate_mean_per_parallel(num_terminals::Int, num_trials::Int)
    ...
    # 出力ディレクトリ作成
    output_dir = "result_parallel_evaluation/optimized"
    mkpath(output_dir)
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # 出力ディレクトリ作成
    out_dir = "result_parallel_evaluation/Prop_Async_Optimized"
    mkpath(out_dir)
    ...
    open(param_file, "w") do io
        ...
        println(io, "SF: $(SPREADING_FACTOR)")
        println(io, "ACK Enabled: $(ENABLE_ACK)")
