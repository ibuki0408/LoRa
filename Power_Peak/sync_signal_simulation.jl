...
function deploy_terminals(deployment_params::TerminalDeploymentParameters, shadowing_params::ShadowingParameters, tx_power_dbm::Float64)
    ...
    elseif deployment_params.deployment_mode == "fixed"
        ...
        println("固定端末数モード: 端末数$(num_points)")
        
    else
        error("不正な配置モード: $(deployment_params.deployment_mode)")
    end
    
    # 円内にランダム配置（基地局を中心とした円）
...
function main()
    ...
    println("• 帯域幅: $(sim_params.bandwidth_mhz) MHz")
    ...
    params = SignalParameters(
        ...
        sim_params.bandwidth_mhz * 1e6,        # MHz → Hz
    ...
    # 熱雑音密度: -174 dBm/Hz
    noise_figure_db = 5.0
    fixed_noise_power_dbm = -174 + 10 * log10(params.bandwidth_hz) + noise_figure_db
    ...
    # 出力ディレクトリを作成
    output_dir = "results_sync_simulation"
    mkpath(output_dir)
