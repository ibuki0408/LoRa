...
function generate_random_ofdm_signal(bandwidth_hz::Float64, duration_s::Float64, fs_hz::Float64)
    ...
    # 窓関数を適用して境界での不連続性を抑制
    window = hanning(bw_bins + 1)  # ハニング窓
    
    for (idx, i) in enumerate(start_bin:end_bin)
        # 窓関数を適用したランダムな複素数を生成
        freq_domain_signal[i] = rand(ComplexF64) * window[idx]
    end
    ...
    # 時間領域でも窓関数を適用して境界でのピークを抑制
    time_window = hanning(N)
    time_domain_signal_complex = time_domain_signal_complex .* time_window
...
function plot_signal_comprehensive(signal_complex::Vector{ComplexF64}, sampling_rate::Float64, params::SignalParameters)
    # 時間領域プロット
    time_plot = plot_signal_time_domain(signal_complex, sampling_rate, "OFDM-like Signal")
    
    # 周波数領域プロット
    freq_plot = plot_signal_frequency_domain(signal_complex, sampling_rate, "OFDM-like Signal")
...
function save_signal_to_png(signal_complex::Vector{ComplexF64}, sampling_rate::Float64, params::SignalParameters, output_dir::String)
    ...
    # PNGファイルに保存
    savefig(combined_plot, "$(output_dir)/signal_visualization.png")
...
function main(output_mode::String="csv")
    println("="^60)
    if output_mode == "csv"
        println("OFDMライク信号生成・CSV出力プログラム")
    elseif output_mode == "png"
        println("OFDMライク信号生成・PNG出力プログラム")
    else
        println("OFDMライク信号生成・CSV+PNG出力プログラム")
    end
    println("="^60)
    ...
    signal_complex = generate_random_ofdm_signal(
        params.bandwidth_hz,
        params.duration_s,
    ...
    # 出力ディレクトリを作成
    output_dir = "results_random_frequency"
    mkpath(output_dir)
    ...
    if output_mode == "png" || output_mode == "both"
        ...
        println("• $(output_dir)/signal_visualization.png - 信号可視化")
        println()
    end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    if length(ARGS) > 0
        ...
        if output_mode in ["csv", "png", "both"]
            ...
            println("使用方法: julia ramdom_frequency_plot.jl [csv|png|both]")
